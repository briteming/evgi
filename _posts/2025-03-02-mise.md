...
可以使用如下的命令查看 mise 支持的所有工具

```
mise registry
```

该命令会列出注册表中所有的可用工具，包括简写名以及对应的后端，比如常见的 node, jq, aws-cli 等等。还可以使用 `mise search <keyword>` 来检索特定的工具。
...
不带参数的 `mise use` 会打开一个 TUI 终端交互界面，可以在列表中浏览，搜索并选择要安装的工具。
